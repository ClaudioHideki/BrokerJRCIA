...
UI: `/jrc` apresenta empresas, limites, usuários, indicadores e a aba JRC Conversas; SUPPORT não recebe edição administrativa. `/integracoes` permite vincular conta, caixas, atendentes e consultar entregas. `/whatsapp-oficial` orienta autorização/pendências; `/conexoes` orienta QR e reconexão; `/mensagens` oferece texto, anexos recebidos, templates e a integração Typebot existente. Nesta release, criação e execução de novos fluxos ficam no módulo Flows do **JRC Conversas**. Desative outra automação no canal antes de ativar Flows na mesma caixa. Consulte `jrc-conversas-release.md` para configuração conjunta; o canvas experimental no Broker fica para uma versão posterior.
