...
Gere valores diferentes para cada segredo, com pelo menos 32 caracteres aleatórios; senhas usadas em URL devem ser hexadecimais ou corretamente percent-encoded. As chaves MFA, de integração, Meta e cada valor versionado em `CREDENTIAL_VAULT_KEYS_JSON` devem ser distintos, codificados em base64 canônico de exatamente 32 bytes aleatórios. Preserve versões antigas do cofre enquanto houver credenciais cifradas com elas. `AUTOMATION_SANDBOX_TOKEN` é um token independente com pelo menos 32 caracteres aleatórios; o Compose o compartilha entre sandbox e worker de I/O. Guarde-os em cofre e no backup de segredos, nunca em Git ou no build do frontend. `.env.example` não contém credenciais funcionais.

## Preparação e instalação futura

1. Após revisão e autorização de publicação, execute o workflow manual `images.yml` no ambiente GitHub `ghcr-release`; `publish=false` é o padrão. `publish=true` publica tags da revisão Git em GHCR, gera SBOM e provenance pelo BuildKit e assina os dois digests com a identidade OIDC do workflow. Revisores adicionais podem ser configurados nesse ambiente conforme a política da equipe. Configure o segredo de repositório `AUDIT_FINGERPRINT_SECRET` com valor aleatório exclusivo para a CI. Não há webhook de deploy automático. Execute também a CI de integração/E2E antes da liberação; o job de imagens não substitui essa CI.
...
3. Crie aplicação Docker Compose e carregue os arquivos de `infra/dokploy`. Preencha o ambiente. Defina um domínio HTTPS para o serviço **web**, porta **8080**, pela aba Domains. Configure `BROKER_HOST` apenas com o host desse domínio (sem `https://`, porta ou caminho) e `PUBLIC_ORIGIN` com a origem HTTPS completa correspondente, sem barra final. Os dois roteadores Traefik do Compose usam `BROKER_HOST`; se omitido, mantêm `jrc-broker.jrcws.cloud`. Se instalar mais de uma stack Broker no mesmo Docker/Traefik, defina `BROKER_STACK_NAME` e `BROKER_ROUTE_ID` únicos por stack; omiti-los preserva os identificadores atuais. Em instalações com somente Chatwoot externo, deixe `CHATWOOT_BASE_URL` e `CHATWOOT_PLATFORM_TOKEN` vazios, configure a chave `INTEGRATION_ENCRYPTION_KEY` e habilite `CHATWOOT_EXTERNAL_DESTINATIONS_ENABLED=true` e `CHATWOOT_CONTROL_ENABLED=true` somente depois da validação do destino. Para uma instalação gerenciada JRC Conversas, informe a origem HTTPS em `CHATWOOT_BASE_URL`. Não aponte domínio ao PostgreSQL, Redis ou motor.
...
5. Crie o primeiro administrador com `npm run platform:admin:create` dentro da imagem, usando credenciais administrativas em ambiente temporário. Em Dokploy use `PLATFORM_LOGIN_MODE=password_totp`: o comando imprime uma única URI `otpauth` para cadastro no autenticador e o login passa a exigir senha e TOTP. `PLATFORM_LOGIN_MODE=password` fica restrito a desenvolvimento local explicitamente autorizado. Configure `PLATFORM_ORIGIN=PUBLIC_ORIGIN` com HTTPS e remova as variáveis da senha inicial após criar o administrador.
...
UI: `/jrc` apresenta empresas, limites, usuários, indicadores e a aba JRC Conversas; SUPPORT não recebe edição administrativa. `/integracoes` permite vincular conta, caixas, atendentes e consultar entregas. `/whatsapp-oficial` orienta autorização/pendências; `/conexoes` orienta QR e reconexão; `/mensagens` oferece texto, anexos recebidos, templates e a integração Typebot existente. O candidato de 18/09 acrescenta `/flows` ao Broker, com execução direta no WhatsApp ou por Agent Bot em caixas JRC/Chatwoot. O motor nativo do JRC continua disponível. Escolha um único motor por caixa e siga [o guia atualizado](broker-omnichannel.md), incluindo seus limites e a homologação pendente.
...
Antes de cada release execute `npm run test:restore-drill`. O exercício sobe uma composição descartável, cria duas organizações sintéticas com RLS, banco de sessão do motor e arquivo de sessão, produz backup cifrado, verifica autenticação e checksums, restaura os dois bancos e confirma que o papel comum continua sem `BYPASSRLS`. A evidência sanitizada fica em `.sessions/restore-drill.json`. Os tempos medidos localmente são evidência do exercício, não promessa de RTO do servidor; meça novamente com o volume e a infraestrutura de produção.
