...
export function InstanceWorkspace({ instanceId }: { instanceId: string }) {
  ...
  return <div className="instance-workspace">
    ...
    {tab === 'Integrações' ? <section className="panel"><h2>Integrações JRC</h2><div className="workspace-integrations"><article><h3>Mensagens e Typebot</h3><p>Typebot está disponível nos canais Meta. A automação e a caixa de entrada desta conexão por QR Code ainda não estão disponíveis.</p><Link to="/mensagens">Abrir mensagens e automações</Link></article><article><h3>WhatsApp oficial</h3><p>Gerencie os canais autorizados pela Meta na organização ativa.</p><Link to="/whatsapp-oficial">Gerenciar WhatsApp oficial</Link></article><article><h3>API da organização</h3><p>Gerencie as chaves e permissões de acesso à API JRC.</p><Link to="/chaves-api">Gerenciar chaves de API</Link></article></div><p className="workspace-help">Webhooks de saída, Chatwoot, n8n e outros adaptadores ainda não possuem configuração nesta console.</p></section> : null}
  </div>;
}
