...
const steps = ['Provedor', 'Conta', 'Canal', 'Destino', 'Automação', 'Revisão'];
export function NewChannelPage() {
  ...
  return <section aria-labelledby="new-channel-title"><Link className="back-link" to="/channels">← Voltar para canais</Link>
    ...
    {!provider ? <div className="provider-cards"><button className="panel provider-card" onClick={() => { setProvider('qr'); setQuery({ provider: 'qr' }, { replace: true }); }}>
      <strong>WhatsApp por QR Code</strong><span>Conecte um aparelho WhatsApp Business pela Evolution API.</span></button>
