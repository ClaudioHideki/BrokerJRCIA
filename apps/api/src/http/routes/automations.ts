...
export async function registerAutomationRoutes(app:FastifyInstance,options:AutomationRouteOptions){
  ...
  app.setErrorHandler((error,request,reply)=>{if(error instanceof AutomationError)return problem(reply,request,error.statusCode,error.code,error.details);
  ...
  if(options.migration){const migration=options.migration;
   ...
   api.post('/v1/automations/migrations/legacy',{preHandler:write,schema:{headers:IdempotencyHeadersSchema,querystring:empty,body:z.strictObject({limit:z.number().int().min(1).max(200).default(50)})}},request=>migration.migrateBatch(org(request),{limit:request.body.limit,actorId:request.authentication!.actorId!}));
  ...
  api.patch('/v1/automations/:id/bindings/:bindingId',{preHandler:write,schema:{params:bindingParams,querystring:empty,body:z.strictObject({status:z.enum(['ACTIVE','PAUSED','DISABLED']),revision:z.number().int().positive()})}},request=>options.service.setBindingStatus(org(request),request.params.bindingId,request.body));
